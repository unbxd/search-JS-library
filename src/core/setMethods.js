...
const extraActions = function(e) {
	...
	const { viewMoreText, textFacetWrapper } = this.options.facet;
	if (dataset) {
		const { facetAction, facetName, action } = dataset;
		const { facetElementMap } = this.viewState;
...
const extraActionsChange = function(e) {
	...
	if (dataset) {
		const { facetAction, facetName } = dataset;
