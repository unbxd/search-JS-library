...
const options = {
    ...
    noResults: {
        template: function (query) { return `${window.unbxdImageSearch && window.unbxdImageSearch.isVisualSearchEnabled() ? `<div class="UNX-no-results">  No discernible products found </div>` : `<div class="UNX-no-results"> No Results found ${query} </div>`}` }
    },
    onEvent: (state, type) => {
